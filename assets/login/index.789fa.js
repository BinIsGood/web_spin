...
System.register("chunks:///_virtual/LoginPage.ts", ['./rollupPluginModLoBabelHelpers.js', 'cc', './Constant.ts', './BaseEntryView.ts', './GameMgrs.ts', './UIHelper.ts', './NetConstant.ts', './NGame.ts', './BackgroundMainLoading.ts', './GameDataMgr.ts', './CooldownManager.ts', './HttpPbFunc.ts', './WebApiEnums.ts', './PlatformUtils.ts', './Decorators.ts', './EncryPlus.ts', './LoginUtils.ts', './GoogleAnatytics.ts', './LogReport.ts', './TestJS.ts'], function (exports) {
  var _applyDecoratedDescriptor, _inheritsLoose, _initializerDefineProperty, _assertThisInitialized, _extends, _asyncToGenerator, _regeneratorRuntime, cclegacy, _decorator, Node, UIOpacity, tween, Sprite, EditBox, isValid, BundleName, StorageKey, EventName, UserLoginType, BaseEntryView, gameMgrs, UIHelper, CMD_SUCCESS, NGame, backgroundMainLoading, gameDataMgr, CooldownManager, HttpPbFunc, pbfixUserLoginType, PlatformUtils, inject, encryPlus, LoginUtils, GoogleAnalytics, GAEvent, LogReport, TestJS;
  return {
    setters: [function (module) {
      ...
      LogReport = module.LogReport;
    }, function (module) {
      TestJS = module.default;
    }],
    execute: function () {
      ...
      var LoginPage = exports('default', (_dec = ccclass('LoginPage'), _dec2 = inject("node_reg/btn_reg/", Node), _dec3 = inject("node_list/view/node_content/node_remenber/btn_find_pwd/", Node), _dec4 = inject("node_list/view/node_content/node_pwd/btn_pwd_show/", Node), _dec5 = inject("node_list/view/node_content/node_pwd/btn_pwd_show/icon_pwd_hide/", Node), _dec6 = inject("node_list/view/node_content/node_pwd/btn_pwd_show/icon_pwd_show/", Node), _dec7 = inject("node_list/view/node_content/node_remenber/node_remenber/", Node), _dec8 = inject("node_list/view/node_content/node_remenber/node_remenber/btn_checkbox/checkbox/icon_sel/", Node), _dec9 = inject("node_list/view/node_content/node_username/EditBox/", Node), _dec10 = inject("node_list/view/node_content/node_pwd/EditBox/", Node), _dec(_class = (_class2 = (_class3 = /*#__PURE__*/function (_BaseEntryView) {
        ...
        _proto.open = function open() {
          ...
          // 占位符场景:焦点进入即清空 ********,避免与用户输入混在一起;失焦无输入则恢复占位符 + 强制密文显示
          this.pwd1EditBoxNode.on('editing-did-began', this.onPwdEditingDidBegan, this);
          this.pwd1EditBoxNode.on('editing-did-ended', this.onPwdEditingDidEnded, this);
          this.addClickEvent();
        ...
        _proto.iniveView = function iniveView() {
          ...
          // 本地有 hash 或残留明文 → 用占位符填充密码框,真实凭据保存在 storage 里
          if (this.hasLocalSavedPwd()) {
            this.fillPwdPlaceholder();
          } else {
            this.pwd1EditBox.string = '';
            this._pwdIsPlaceholder = false;
          }
          this.userNameEditBoxFunc(this.userNameEditBox);
        }

        /** 本地是否存在已保存的密码凭据(新版 hash 或历史明文都算) */;
        _proto.hasLocalSavedPwd = function hasLocalSavedPwd() {
          var hash = NGame.storage.get(StorageKey.INPUT_PASSWORD_HASH, '');
          if (hash && hash.length > 0) return true;
          var legacy = NGame.storage.get(StorageKey.INPUT_PASSWORD, '');
          return !!legacy && legacy.length > 0;
        }

        /** 清除占位符状态(用户开始输入/切换显示等场景调用) */;
        _proto.clearPwdPlaceholder = function clearPwdPlaceholder() {
          if (!this._pwdIsPlaceholder) return;
          this._pwdIsPlaceholder = false;
          this.pwd1EditBox.string = '';
        }

        /** 用占位符填充密码框,并强制切回密码模式(隐藏小眼睛图标同步) */;
        _proto.fillPwdPlaceholder = function fillPwdPlaceholder() {
          this.pwd1EditBox.string = LoginPage.PWD_PLACEHOLDER;
          this._pwdIsPlaceholder = true;
          this.pwd1EditBox.inputFlag = EditBox.InputFlag.PASSWORD;
          this.pwdHideTag.active = true;
          this.pwdShowTag.active = false;
        }

        /** 密码框获得焦点:占位符状态下立即清空,避免用户在 ******** 中间插入字符 */;
        _proto.onPwdEditingDidBegan = function onPwdEditingDidBegan() {
          if (this._pwdIsPlaceholder) {
            this.clearPwdPlaceholder();
            this.userNameEditBoxFunc(this.pwd1EditBox);
          }
        }

        /** 密码框失去焦点:若无任何输入且本地存在已保存的密码凭据,恢复占位符 + 强制密文显示 */;
        _proto.onPwdEditingDidEnded = function onPwdEditingDidEnded() {
          if (this._pwdIsPlaceholder) return;
          var isEmpty = this.pwd1EditBox.string.trim().length === 0;
          if (isEmpty && this.hasLocalSavedPwd()) {
            this.fillPwdPlaceholder();
            this.userNameEditBoxFunc(this.pwd1EditBox);
          }
        };
        ...
        _proto.userNameEditBoxFunc = function userNameEditBoxFunc(editBox) {
          // 占位符场景:如果用户改动了密码框(内容已不等于占位符),清掉占位符进入"重新输入"分支
          if (this._pwdIsPlaceholder && this.pwd1EditBox.string !== LoginPage.PWD_PLACEHOLDER) {
            this.clearPwdPlaceholder();
          }
          var userName = this.userNameEditBox.string.trim();
          // 占位符状态下,视作"已有合法密码",不参与长度校验
          var pwdOk = this._pwdIsPlaceholder || this.pwd1EditBox.string.trim().length >= 6;
          this.allNode.loginBtn.getComponent(Sprite).grayscale = userName.length < 6 || !pwdOk;
        };
        ...
        _proto.verifyInput = function verifyInput() {
          ...
          // 正则：开头到结尾，只能是 字母/数字/下划线，长度 5~20
          ...
          // 占位符状态:本地已有 hash/明文,可直接登录,跳过密码长度校验
          if (this._pwdIsPlaceholder) {
            return true;
          }
        ...
        _proto.accountLogin = function accountLogin() {
          ...
          var md5Password;
          if (this._pwdIsPlaceholder) {
            // 优先用本地 hash;若没有(历史用户只有明文),即时 hash 一次,登录成功后由 loginCallback 写入 hash 并清理明文
            md5Password = NGame.storage.get(StorageKey.INPUT_PASSWORD_HASH, '');
            if (!md5Password) {
              var legacy = NGame.storage.get(StorageKey.INPUT_PASSWORD, '');
              if (!legacy) {
                // 极端兜底:占位符状态但本地凭据被清空,要求重新输入
                this.clearPwdPlaceholder();
                NGame.tips.toast("Please enter your password".i18nStr());
                return;
              }
              md5Password = encryPlus.hash256(legacy);
            }
          } else {
            var oriPassword = this.pwd1EditBox.string.trim();
            md5Password = encryPlus.hash256(oriPassword); // hash 256
          }
          ...
          var _onLogined = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee2(result) {
            return _regeneratorRuntime().wrap(function _callee2$(_context2) {
              while (1) switch (_context2.prev = _context2.next) {
                case 0:
                  LogReport.reportPWA({
                    msg: 'loginpage res:' + ((result == null ? void 0 : result.pbfixCode) || '')
                  });
        ...
        /**
         * 从重置密码页面回来,刷新密码输入框
         * 重置密码成功后本地会写入新的 hash,这里重新走一遍占位符填充
         */;
        ...
        _proto.onClickPwdShowTypeBtn = function onClickPwdShowTypeBtn() {
          // 占位符状态下,切换显示模式之前必须先清空占位符内容,否则会暴露 8 个真实"*"——虽然不是密码,但会让用户误解
          if (this._pwdIsPlaceholder) {
            this.clearPwdPlaceholder();
            this.userNameEditBoxFunc(this.pwd1EditBox);
          }
        ...
        return LoginPage;
      }(BaseEntryView), _class3.PWD_PLACEHOLDER = "********", _class3), (_descriptor = _applyDecoratedDescriptor(_class2.prototype, "regBtn", [_dec2], {
        configurable: true,
        enumerable: true,
        writable: true,
  ...
  return {
    ...
    execute: function () {
      ...
      var LoginUtils = exports('LoginUtils', /*#__PURE__*/function () {
        function LoginUtils() {}
        /**
         * 验证用户名是否符合规则， 5-20位, 只能字母/数字/下划线，并且以字母开头
         * @param account 
         * @returns 
         */
...
System.register("chunks:///_virtual/RegDialog.ts", ['./rollupPluginModLoBabelHelpers.js', 'cc', './Constant.ts', './Decorators.ts', './EncryPlus.ts', './MKDialogFitViewBase.ts', './NetConstant.ts', './NGame.ts', './BackgroundMainLoading.ts', './GameDataMgr.ts', './HttpPbFunc.ts', './PlatformUtils.ts', './LoginUtils.ts', './MKExport.ts', './GoogleAnatytics.ts', './LogReport.ts'], function (exports) {
  var _applyDecoratedDescriptor, _inheritsLoose, _initializerDefineProperty, _assertThisInitialized, _extends, _asyncToGenerator, _regeneratorRuntime, cclegacy, _decorator, Node, tween, UIOpacity, EditBox, UITransform, Label, sys, UserLoginType, StorageKey, EventName, BundleName, inject, encryPlus, MKDialogFitViewBase, CMD_SUCCESS, NGame, backgroundMainLoading, gameDataMgr, HttpPbFunc, isResponseValid, PlatformUtils, LoginUtils, gloEvent, GoogleAnalytics, GAEvent, LogReport;
  return {
    setters: [function (module) {
      _applyDecoratedDescriptor = module.applyDecoratedDescriptor;
      _inheritsLoose = module.inheritsLoose;
      ...
      MKDialogFitViewBase = module.MKDialogFitViewBase;
    }, function (module) {
      CMD_SUCCESS = module.CMD_SUCCESS;
    }, function (module) {
      NGame = module.NGame;
    ...
    execute: function () {
      ...
      var RegDialog = exports('RegDialog', (_dec = ccclass('RegDialog'), _dec2 = inject("content/node_contain/btn_close/", Node), _dec3 = inject("content/node_contain/label_title/", Node), _dec4 = inject("content/node_contain/", Node), _dec5 = inject("content/node_contain/node_content/", Node), _dec6 = inject("content/node_contain/node_content/regBtn/", Node), _dec7 = inject("content/node_contain/node_content/guestBtn/", Node), _dec8 = inject("content/node_contain/node_content/node_agreen/", Node), _dec9 = inject("content/node_contain/node_content/or/", Node), _dec10 = inject("content/node_contain/node_content/node_agreen/btn_checkbox/", Node), _dec11 = inject("content/node_contain/node_content/node_agreen/btn_checkbox/checkbox/icon_sel/", Node), _dec12 = inject("content/node_contain/node_content/node_pwd1/btn_pwd_show/", Node), _dec13 = inject("content/node_contain/node_content/node_pwd1/btn_pwd_show/icon_pwd_hide/", Node), _dec14 = inject("content/node_contain/node_content/node_pwd1/btn_pwd_show/icon_pwd_show/", Node), _dec15 = inject("content/node_contain/node_content/node_pwd2/btn_pwd_show/", Node), _dec16 = inject("content/node_contain/node_content/node_pwd2/btn_pwd_show/icon_pwd_hide/", Node), _dec17 = inject("content/node_contain/node_content/node_pwd2/btn_pwd_show/icon_pwd_show/", Node), _dec18 = inject("content/node_contain/node_content/node_username/EditBox/", Node), _dec19 = inject("content/node_contain/node_content/node_pwd1/EditBox/", Node), _dec20 = inject("content/node_contain/node_content/node_pwd2/EditBox/", Node), _dec21 = inject("content/node_contain/node_content/node_invite_code/", Node), _dec22 = inject("content/node_contain/node_content/node_invite_code/EditBox/", Node), _dec(_class = (_class2 = /*#__PURE__*/function (_MKDialogFitViewBase) {
        ...
        _proto.pbfixBindAccountReq = function pbfixBindAccountReq() {
          ...
          HttpPbFunc.pbfixBindAccountReq(this.node, params, function (result) {
            if (isResponseValid(result, "Bind account failed".i18nStr())) {
              ...
              _this5.saveAccountInfo(account, md5Password);
        ...
        _proto.saveAccountInfo = function saveAccountInfo(account, md5Password) {
          NGame.storage.set(StorageKey.INPUT_USERNAME, account);
          NGame.storage.set(StorageKey.INPUT_PASSWORD_HASH, md5Password);
          // 清理历史明文密码(线上存量淘汰)
          NGame.storage.remove(StorageKey.INPUT_PASSWORD);
        }
        ...
        _proto.onReged = function onReged(result) {
          LogReport.reportPWA({
            msg: 'reg res:' + ((result == null ? void 0 : result.pbfixCode) || '')
          });
          if ((result == null ? void 0 : result.pbfixCode) != CMD_SUCCESS) {
          ...
          var _onLogined = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee2(result) {
            return _regeneratorRuntime().wrap(function _callee2$(_context2) {
              while (1) switch (_context2.prev = _context2.next) {
                case 0:
                  LogReport.reportPWA({
                    msg: 'login-reg res:' + ((result == null ? void 0 : result.pbfixCode) || '')
                  });
                  if (!((result == null ? void 0 : result.pbfixCode) != CMD_SUCCESS)) {
                    _context2.next = 6;
                    break;
        ...
        return RegDialog;
      }(MKDialogFitViewBase), (_descriptor = _applyDecoratedDescriptor(_class2.prototype, "closeBtn", [_dec2], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
...
System.register("chunks:///_virtual/ResetPwdDialog.ts", ['./rollupPluginModLoBabelHelpers.js', 'cc', './Constant.ts', './Decorators.ts', './EncryPlus.ts', './MKDialogFitViewBase.ts', './NetConstant.ts', './NGame.ts', './CooldownManager.ts', './HttpPbFunc.ts', './PlatformUtils.ts', './LoginUtils.ts', './GameDataMgr.ts'], function (exports) {
  var _applyDecoratedDescriptor, _inheritsLoose, _initializerDefineProperty, _assertThisInitialized, _asyncToGenerator, _regeneratorRuntime, cclegacy, _decorator, Node, UITransform, Label, EditBox, StorageKey, inject, encryPlus, MKDialogFitViewBase, CMD_SUCCESS, NGame, CooldownManager, HttpPbFunc, isResponseValid, PlatformUtils, LoginUtils, gameDataMgr;
  return {
    setters: [function (module) {
      _applyDecoratedDescriptor = module.applyDecoratedDescriptor;
      _inheritsLoose = module.inheritsLoose;
      ...
      var ResetPwdDialog = exports('ResetPwdDialog', (_dec = ccclass('ResetPwdDialog'), _dec2 = inject("content/btn_close/", Node), _dec3 = inject("content/label_title/", Node), _dec4 = inject("content/node_content/confirmBtn/", Node), _dec5 = inject("content/node_content/node_tips/btn_help/", Node), _dec6 = inject("content/node_content/node_sms/smsBtn/", Node), _dec7 = inject("content/node_content/node_username/", Node), _dec8 = inject("content/node_content/node_username/EditBox/", Node), _dec9 = inject("content/node_content/node_pwd_origin/", Node), _dec10 = inject("content/node_content/node_pwd_origin/EditBox/", Node), _dec11 = inject("content/node_content/node_pwd1/EditBox/", Node), _dec12 = inject("content/node_content/node_pwd2/EditBox/", Node), _dec13 = inject("content/node_content/node_phone/", Node), _dec14 = inject("content/node_content/node_phone/EditBox/", Node), _dec15 = inject("content/node_content/node_sms/", Node), _dec16 = inject("content/node_content/node_sms/smsBg/EditBox/", Node), _dec17 = inject("content/node_content/node_pwd_origin/btn_pwd_show/", Node), _dec18 = inject("content/node_content/node_pwd_origin/btn_pwd_show/icon_pwd_hide/", Node), _dec19 = inject("content/node_content/node_pwd_origin/btn_pwd_show/icon_pwd_show/", Node), _dec20 = inject("content/node_content/node_pwd1/btn_pwd_show/", Node), _dec21 = inject("content/node_content/node_pwd1/btn_pwd_show/icon_pwd_hide/", Node), _dec22 = inject("content/node_content/node_pwd1/btn_pwd_show/icon_pwd_show/", Node), _dec23 = inject("content/node_content/node_pwd2/btn_pwd_show/", Node), _dec24 = inject("content/node_content/node_pwd2/btn_pwd_show/icon_pwd_hide/", Node), _dec25 = inject("content/node_content/node_pwd2/btn_pwd_show/icon_pwd_show/", Node), _dec(_class = (_class2 = /*#__PURE__*/function (_MKDialogFitViewBase) {
        ...
        _proto.initView = function initView() {
          ...
          if (this.isChangePwdDialog) {
            ...
            this.pwd1EditBox.placeholder = 'Please enter a password'.i18nStr();
          }
        };
        _proto.addClickEvent = function addClickEvent() {
          var _this2 = this;
        ...
        _proto.onResetPwdFinished = /*#__PURE__*/function () {
          var _onResetPwdFinished = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(result, md5Password) {
            var _this$initData2;
            return _regeneratorRuntime().wrap(function _callee$(_context) {
              while (1) switch (_context.prev = _context.next) {
                case 0:
        ...
        _proto.pbfixResetPasswordReq = function pbfixResetPasswordReq() {
          ...
          HttpPbFunc.pbfixResetPasswordReq(this.node, reqParams, function (result) {
            _this5.onResetPwdFinished(result, md5Password);
          });
        };
        _proto.savePwd = function savePwd(md5Password) {
